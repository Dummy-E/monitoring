...
# Running file beat agent on any server
  ...
  docker run -d --name=filebeat   --restart unless-stopped --user=root  -v /var/lib/docker/containers:/var/lib/docker/containers:ro   -v $(pwd)/orchestration/filebeat.yml:/usr/share/filebeat/filebeat.yml   docker.elastic.co/beats/filebeat:8.6.2 filebeat -e
else
